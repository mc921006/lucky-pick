...
import { isSpeettoCacheFresh, loadSpeettoData, syncSpeettoData } from "@/lib/speetto/sync";
...
export async function GET() {
  ...
  try {
    ...
    if (isSpeettoCacheFresh(storedProducts)) {
      return NextResponse.json({ success: true, source: "cache", products: storedProducts });
    }
    ...
    return NextResponse.json({
      success: false,
      message: error instanceof Error ? error.message : "스피또 정보를 가져오지 못했습니다.",
    }, { status: 502 });
  }
}
